...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    handlers=[logging.FileHandler("training_data_gathering.log"),
...
def list_audio_devices():
    ...
    """
    logging.info("Listing audio devices")
    p = pyaudio.PyAudio()
    ...
    for i in range(0, num_devices):
        ...
        if device_info.get('maxInputChannels') > 0:
            logging.debug(f"Input Device id {i} - {device_info.get('name')}")
    p.terminate()
...
def create_directory_if_not_exists(directory):
    ...
    if not os.path.exists(directory):
        ...
        logging.debug(f"Directory already exists at {directory}")
...
def record_audio(filename, duration, rate=44100, channels=2, chunk_size=1024, device_index=None):
    ...
    device_index: index number to specify which audio device to use. "1" will use the default audio device
        selected by the OS
    """
    logging.info(f"Starting audio recording: {filename} for {duration} seconds")
    try:
        # Initialize pyaudio
        p = pyaudio.PyAudio()
        ...
        logging.info(f"Audio recording saved to {filename}")
...
def main():
    ...
    timestamp: current time stamp at the time of execution. It is appended to the end of the file name to
